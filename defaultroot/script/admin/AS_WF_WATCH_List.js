...
function setPageInit(){
	voGrid = PageX.getCtrlObj("WF_WATCH_Grid");
	voGrid.addListener(new Listener(voGrid.OnRowDblClick, instancetrace2, this));  		
	
	voCombobox = PageX.getCtrlObj("templatename");
  voCombobox.addListener(new Listener(voCombobox.OnChange, VS_OnChange, this));  		
  ...
  voToolbar.addListener(new Listener(voToolbar.OnCallClick, eventAnswer_Toolbar_OnCallClick, this));
	...
	voSearch = PageX.getCtrlObj("AS_WF_WATCH_search");
...
function instancetrace(){
  ...
  if (aiRow.length>=2 ) {
	...
	WFPageX = new WFPage4();
	var intanceId = voGrid.getValue("WF_INSTANCE_ID");
    WFPageX.showInstanceTrace(intanceId);
	}
	
//双击流程跟踪oRow.rowIndex
function instancetrace2(){
	var intanceId = voGrid.getValue("WF_INSTANCE_ID");
	WFPageX = new WFPage4();
    WFPageX.showInstanceTrace(intanceId);
	}
