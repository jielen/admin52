/* $Id: TreeView2.js,v 1.3 2008/06/12 08:18:05 hemg Exp $ */
/*
...
function TreeView(){
  ...
  // CW5.2升级 xiexx
  this.NORM_IMG_PAR_NODE= PageX.localResPath + "/style/img/gp5/ico/company.gif";
  this.NORM_IMG_NODE= PageX.localResPath + "/style/img/gp5/ico/leafdark_16x16.gif";
  ...
  this.eventAnswer_OnMouseUp= TreeView_eventAnswer_OnMouseUp;
  ...
  // CW5.2升级 xiexx
  this.nodeData = TreeView_nodeData;
  this.getEditNodeData = TreeView_getEditNodeData;
  this.saveNode = TreeView_saveNode;
  this.getIsLowestField = TreeView_getIsLowestField;
  this.gotoNode = TreeView_gotoNode;
  this.insertMoreNodesForChildNodes = TreeView_insertMoreNodesForChildNodes;
  this.insertMoreNode = TreeView_insertMoreNode;
  this.reload = TreeView_reload;
}
//----------------------------------------------------------------------
//6.方法区 =function();
//----------------------------------------------------------------------
...
function TreeView_makeMoreNode(sPCode){
  ...
  if(sPCode != null && sPCode.indexOf("EMP_") > 0) return;
  var voBuf= new StringBuffer();
...
function TreeView_makeNodeTextBegin(sCode, sName, tIsCheck, tIsCodeAndCap,
  ...
  //PageX.showMessage("", voBuf.toString());
  return voBuf.toString();
}
//----------------------------------------------------------------------
//pvivate; 生成构件的HTML对象.
...
//7.事件响应区 =function();
//----------------------------------------------------------------------
//private; Base 的事件响应; OnMouseUp;
//return: void;
function TreeView_eventAnswer_OnMouseUp(oSender, oEvent){
...
//----------------------------------------------------------------------
// CW5.2升级 xiexx
function TreeView_getEditNodeData(sNodeCode, sNodeName, sPNodeCode, sIsLowest) {
  var vaoNodeData= new Array();
  var voData= new this.nodeData();
  voData.sNodeCode= sNodeCode;
  voData.sPNodeCode= sPNodeCode;
  voData.sIsLowest= sIsLowest;
  voData.sCaption= "["+ voData.sNodeCode+ "] " + sNodeName;
...
//----------------------------------------------------------------------
// CW5.2升级 xiexx
function TreeView_saveNode(sNodeCode, sNodeName, sPNodeCode, sIsLowest) {
  //alert("TreeView_saveNode");
  var vaoNodeData= this.getEditNodeData(sNodeCode, sNodeName, sPNodeCode, sIsLowest);
  if (vaoNodeData== null || vaoNodeData.length== 0) return;
   for (var i= 0; i< vaoNodeData.length; i++){
    var vsLeafImg= this.IMG_LEAF_NORMAL;
    if (vaoNodeData[i].sIsLowest == "Y"){
      vsLeafImg= this.NORM_IMG_NODE;
    } else {
      vsLeafImg= this.NORM_IMG_PAR_NODE;
    }
    
    var vasPCode= vaoNodeData[i].sPNodeCode;

    if (!PF.isEmpty(vasPCode)){
      this.collapse(vasPCode);
    }

    var voPNode= this.getNode(vasPCode, true);
    if (voPNode== null){
      voPNode= this.getRoot();
    }

    var vsCode= vaoNodeData[i].sNodeCode;

    if (PF.isEmpty(vsCode)) return;
    
    this.deleteNode(vsCode);
    var voNode= this.makeNode(vsCode, vaoNodeData[i].sCaption, null, null, vsLeafImg, vsLeafImg);
    this.insertChild(voPNode, voNode);
    this.setCurNode(voNode);
    voNode.firstChild.style.color= "green";

    this.setNodeName(vsCode, vaoNodeData[i].sCaption);
    this.setNodeValue(vsCode, this.getCodeField(), vaoNodeData[i].sAccCode);
    this.setNodeValue(vsCode, this.getPCodeField(), vaoNodeData[i].sParAccCode);
    this.setNodeValue(vsCode, this.getIsLowestField(), vaoNodeData[i].sIsLowest);
    this.setNodeValue(vsCode, this.getNormalImgField(), vsLeafImg);
  }
}
//----------------------------------------------------------------------
// CW5.2升级 xiexx
function TreeView_getIsLowestField(){
  return this.oOuterPanel.islowestfield;
}
//----------------------------------------------------------------------
// CW5.2升级 xiexx
function TreeView_gotoNode(sCode) {
  var result = null;
  var vaoRow= DataTools.getTableData(this.getTableName()).selectNodes("rowset/row");
  if (PF.isValidArray(vaoRow)){
    var vaoCode= DataTools.getTableData(this.getTableName()).selectNodes("rowset/row/" + this.getCodeField());
    for (var i= 0; i< vaoCode.length; i++){
      if (vaoCode[i].text != sCode) continue;
        result= vaoRow[i];
    }
  }
  var vsPCode= this.getRowValue(result, this.getPCodeField());
  if (!PF.isEmpty(vsPCode)){
    this.collapse(vsPCode);
  }
  var vsCode= this.getRowValue(result, this.getCodeField());
  if (!PF.isEmpty(vsCode)){
    this.setCurNode(this.getNode(vsCode, true));
  }
}
//----------------------------------------------------------------------
// CW5.2升级 xiexx
//对于子结点全部加上 more node,假定它们都有子结点;
function TreeView_insertMoreNodesForChildNodes(sNodeCode){
  var vasChildCode= this.getChildCodes(sNodeCode);
  if (vasChildCode!= null){
    for (var i= 0, len= vasChildCode.length; i< len; i++){
    	//ch
    	var voNode= this.getNode(vasChildCode[i]);
      if(this.isCollapse(voNode) || this.hasChildNodesForAppear(voNode) || this.hasChildNodes("More...")){
        continue;
    	}else{
        this.insertMoreNode(this.getNode(vasChildCode[i], false));
      }
    }
  }
}
//----------------------------------------------------------------------
// CW5.2升级 xiexx
function TreeView_insertMoreNode(oPNode){
  if (oPNode== null) return;
  var voRow= this.getNodeRow(this.getNodeCode(oPNode));
  var vsIsLowest= this.getRowValue(voRow, this.getIsLowestField());
  var voMoreNode = null;
  if (vsIsLowest != "Y") {
    voMoreNode = this.makeMoreNode(this.getNodeCode(oPNode));
  }
  var vsMoreHtm= "<table>" + voMoreNode + "</table>";
  var voMore= this.makeNewObjs(vsMoreHtm)[0].firstChild;
  this.insertChildToTree(oPNode, voMore);
}
//----------------------------------------------------------------------
// CW5.2升级 xiexx
function TreeView_reload(sCondition){
  var sTableName = this.getTableName();
  var sCompoName = DataTools.getCompoNameByTable(sTableName);
  
  var vasName= new Array("entityName", "condition");
  var vasValue= new Array(sCompoName, sCondition);
  var vsData= requestDataK("geteditpagedata5", sCompoName, vasName, vasValue);
  
  var voXml= PF.parseXml(vsData);
  var voTableXml= voXml.selectSingleNode("data/"+ sTableName);
  if (voTableXml== null){
    alert("没有查到相关数据！");
    return;
  }
  var voDataXML= DataTools.getDataXML(sTableName);
  voDataXML.loadXML(voTableXml.xml);
  this.oTableData= voDataXML.documentElement;
